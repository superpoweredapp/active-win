...
function windows() {
	...
	const constructChromeAndBraveUrl = (url, browserScheme) => {
		...
		const isInvalid = isEmpty || hasWhiteSpace;
		if (isInvalid) return '';

		const isHttpScheme = url.startsWith('http');
		if (isHttpScheme) return url;

		const isHttpURL = !isBrowserScheme && !isFilePath;
		if (isHttpURL) return `https://${url}`;
